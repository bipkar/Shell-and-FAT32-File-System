...
// Convert input file name or sub-directory name into format from file system image and return their position.
// Function will return MAX_DIRECCTORY_ENTRIES value if the sfile or sub-directory cannot be found.
                                            ...
                                            //the starting cluster number of the file or directory name
    {
        ...
        else
        {
            /*printf("Attribute: %x\nSize: %x\nStarting Cluster Number:%x\nPosition:%x\n",
                   ...
                   , LBAToOffset(dir[position].DIR_FirstClusterLow));*/
            printf("Attribute: %x\nSize: %x\nStarting Cluster Number:%x\n",
                dir[position].DIR_Attr,dir[position].DIR_FileSize,dir[position].DIR_FirstClusterLow);
        }
...
/*
 ...
 //printf("%d, %x, %x, %x\n", offset+position,offset+position,num_bytes, next_offset);
 
 //file_data[BPB_BytsPerSec-1] = '\0';
 //printf("%d, %x, %x, %x\n", offset+position,offset+position,num_bytes, next_offset);
 //printf("Remaining bytes in last cluster = %d\n",rem_byts_last_cluster);
 
 
 fseek(fp,44,SEEK_SET); //Location: Offset byte 44
 fread(&BPB_RootClus,2,1,fp); //Get the cluster number of the first ccluster of the root directory
                                //Only defined for FAT 32 media*/

/*for ( i = 0; i < MAX_DIRECTORY_ENTRIES; i++ )
{
    fseek(fp,offset,SEEK_SET);
    fread(&dir[i],32,32,fp);
    offset += 32;
    printf("%c\n", dir[i].DIR_Name[10]);
    printf("%x\n", dir[i].DIR_Attr);
    printf("%x\n", dir[i].Unused1[0]);
